...
const filteredArrayAge = people.filter((person) => person.age >25);
console.log(filteredArrayAge);

//  2.- Crea un array con la gente que empieza por J.

const filteredArrayName = people.filter((person) => person.name.startsWith("J"));
...
const mapArrayAge = people.map(person => {
  if (person.age > 25) {
    return person;
  } else {
    return null;
...
// .filter(persona => persona !== null); Con esta linea haces que el array no envie Null s

 console.log(mapArrayAge);
 

//  2.- Crea un array con la gente que empieza por J.

const mapArrayName = people.map(person => {
...
const imparNumbers2 = numbers2.filter((num) => num % 2);

console.log(imparNumbers2);
...
const filteredFoodList = foodList.filter((food) => food.isVeggie === true).map(food => 'Que rico ' +food.name, 'me voy a comer!');
...
const resultNumbers3 = numbers3.reduce((num1,num2) => num1 * num2);
console.log(resultNumbers3);
...
const extraStaff = staff.map(person => `${person.name} es ${person.role} y le gusta ${person.hobbies}`);
...
const itemsOver300 = inventory.filter(item => item.price > 300).map(item => item.name);
...
const extraReduce = sentenceElements.reduce((word1, word2) => word1 +" " +word2);
